...
```text
SCEATORIO_AI_E2E_PASS: pairing, 29-tool gateway, stdio MCP, scope, replay, expiry, and revoke verified
```
...
- 24 of the 29 operation paths reach the real Lua dispatcher, with 23 successful headless operations and the private annotation path correctly returning `PLAYER_REQUIRED` without a connected human; the saved-blueprint delete path and the four blueprint-book paths are covered by the static and TypeScript suites instead;
- wrong-force and wrong-surface requests fail closed;
- the compiled stdio server completes MCP initialization, reports exactly 29 tools, and calls `get_session` against the live save;
