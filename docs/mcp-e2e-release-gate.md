...
```text
SCEATORIO_AI_E2E_PASS: pairing, 24-tool gateway, stdio MCP, scope, replay, expiry, and revoke verified
```
...
- all 24 operation paths reach the real Lua dispatcher, with 23 successful headless operations and the private annotation path correctly returning `PLAYER_REQUIRED` without a connected human;
- wrong-force and wrong-surface requests fail closed;
- the compiled stdio server completes MCP initialization, reports exactly 24 tools, and calls `get_session` against the live save;
