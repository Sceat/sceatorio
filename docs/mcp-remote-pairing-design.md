...
| **Credentials live in memory only** — a `Map<tokenId, {sha256(secret), grant}>` | A sidecar restart means "pair again" (30 seconds at the Uplink) for at most 4 people; that is cheaper than a PVC, a writable mount, a backup story, and a file of long-lived secrets on disk |
...
run Lua or RCON, touch another force, exceed the per-player quota, outlive the binding, or survive
one click of Revoke. That bounded write surface is why a first-party bearer is proportionate here.
...
| Per-request re-authorization, `TOKEN_REVOKED` / `TOKEN_EXPIRED` | Works now | `aiGateway.lua:538-540`, checked on every call |
...
| # | File | Change | Gate |
|---|---|---|---|
| 1 | `mcp/src/http/credentials.ts` (new) | in-memory mint / verify (`timingSafeEqual` over SHA-256) / revoke / expiry sweep | unit: verify, wrong secret, unknown id, expired, revoked-then-401 |
...
| 5 | `mcp/src/http.ts` (new entry), `package.json` bin, `mcp/Dockerfile` (new) | distroless, non-root, `readOnlyRootFilesystem` stays true (no writable mount needed) | `npm run check`; `docker run` serves `/healthz` |
...
| 7 | `~/dev/kubernetes` (separate commit, devops pass) | enable the `mcp` sidecar, HTTP bind/port, selector-less Service + EndpointSlice, network policy, image digest | `helmfile template` + `tests/test-chart.sh` extended: bind address is never `0.0.0.0` |
...
OAuth 2.1 / an authorization server · a credential PVC or any persistent secret store · a
