...
The MCP side accepts structured entities, tiles, bounded wire references, and expected outputs; arbitrary entity settings are rejected by the v1 safe subset. The client is not asked to hand-author compressed blueprint Base64. Factorio validates entity/tile prototypes, quality, recipe/category/unlock state, connection targets, wire names, and a numeric connector-ID range, computes build-item cost, and can run an optional non-mutating entity-placement collision check. It does not prove that a specific prototype supports a requested connector, simulate tile collisions, or analyze whole-factory connectivity.
