...
async function checkStdioMcp(grant) {
  ...
  try {
    ...
    assert.equal(listed.result?.tools?.length, 24, JSON.stringify(listed));
...
try {
  ...
  console.log("SCEATORIO_AI_E2E_PASS: pairing, 24-tool gateway, stdio MCP, scope, replay, policy disable, expiry, and revoke verified");
} finally {
