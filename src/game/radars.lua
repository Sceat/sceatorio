function playerChart()
	for _,force in pairs(game.forces) do
		if (force.name ~= "neutral") and (force.name ~= "lobby") and (force.name ~= "player") and (force.name ~= "enemy") then
			for _,force2 in pairs(game.forces) do
				if (force2.name ~= "neutral") and (force2.name ~= "lobby") and (force2.name ~= "player") and (force2.name ~= "enemy") and (force.name ~= force2.name) then
